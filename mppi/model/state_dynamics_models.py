...
class HoverKinematicModel(nn.Module):
    """
    Simple kinematic model for hover task.
    Since the action is already a displacement [dx, dy, dz],
    the dynamics is simply: next_state = state + action
    """
    ...
    def forward(self, state, action):
        """
        Compute next_state = state + action (simple kinematic model)
        :param state: torch tensor of shape (..., 3) - end-effector position [x, y, z]
        :param action: torch tensor of shape (..., 3) - displacement [dx, dy, dz]
        :return: next_state: torch tensor of shape (..., 3)
        """
        return state + action
