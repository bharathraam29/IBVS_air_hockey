...
def hover_cost_function(state, action, target_pose, Q_diag=[100, 100, 100]):
    """
    Compute the state cost for MPPI on a hover task.
    The state represents end-effector position [x, y, z] and the target is a hover position.
    :param state: torch tensor of shape (B, state_size) - end-effector position [x, y, z]
    :param action: torch tensor of shape (B, action_size)
    :param target_pose: torch tensor of shape (3,) containing target hover position [x, y, z]
    :param Q_diag: diagonal weights for position cost [Q_x, Q_y, Q_z]
    :return: cost: torch tensor of shape (B,) containing the costs for each of the provided states
    """
    target_pose = target_pose  # torch tensor of shape (3,) containing (x, y, z)
    cost = None
    # --- Your code here
    state_diff = state - target_pose
    if torch.is_tensor(Q_diag):
