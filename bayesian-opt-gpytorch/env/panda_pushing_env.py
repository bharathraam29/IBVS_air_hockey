...
class PandaHoverEnv(PandaPushingEnv):
    """
    Hover task: Control the end-effector to hover at a target position.
    State: end-effector position [x, y, z]
    Action: end-effector velocity/displacement [dx, dy, dz]
    """

    def __init__(self, debug=False, visualizer=None, render_non_push_motions=True,
                 render_every_n_steps=1, camera_heigh=84, camera_width=84):
        # Don't call super with include_obstacle since we don't need objects
        super().__init__(debug, visualizer, include_obstacle=False, 
                        render_non_push_motions=render_non_push_motions,
        ...
        self.action_space = spaces.Box(
            low=np.array([-max_displacement, -max_displacement, -max_displacement]),
    ...
    def step(self, action):
        ...
        current_ee_pos = self.get_end_effector_pos()
        
        # Compute target position: current + action (displacement)
        target_ee_pos = current_ee_pos + action
        
        # Clip target position to workspace limits
        target_ee_pos = np.clip(target_ee_pos, self.space_limits[0], self.space_limits[1])
        
        # Move end-effector to target position
        self._move_robot_ee(target_ee_pos, render=(self.episode_step_counter % self.render_every_n_steps == 0))
    ...
    def _is_done(self, state):
        # Check if end-effector is out of bounds
        out_of_bounds = not self.observation_space.contains(state)
        
        # Check if end-effector is close to target (within tolerance)
        hover_tolerance = 0.05  # 5cm tolerance (increased from 2cm)
        distance_to_target = np.linalg.norm(state - self.target_state)
        at_goal = distance_to_target < hover_tolerance
        
        # Only return done if we're at goal, not if out of bounds (let controller try to recover)
        # Or if severely out of bounds
        done = at_goal
        if out_of_bounds:
            # Only mark as done if way out of bounds (add some margin)
            margin = 0.1  # 10cm margin
            if (np.any(state < self.space_limits[0] - margin) or 
                np.any(state > self.space_limits[1] + margin)):
                done = True
        return done

    def get_state(self):
        """Return end-effector position [x, y, z] as state"""
        state = self.get_end_effector_pos().astype(np.float32)
        return state

    def set_target_state(self, target_state):
        """Set the target hover position"""
        assert isinstance(target_state, np.ndarray)
