...
class PandaHoverStudy:
    """Study class for hover task optimization"""
    ...
    def __init__(self, epoch, render, logdir, study_name, 
        ...
        self._target_state = np.array([0.5, 0.0, 0.2]) if target_state is None else target_state
        self._opt_type = opt_type
        self.test_params = test_params
        ...
        cost_func = hover_cost_function

        self._optimizer = UnifiedBlackboxOptimizer(opt_type, param_dict, device)

        self._step_scale = step_scale
        self._goal_scale = goal_scale
        self._goal_tol = 0.02  # 2cm tolerance for hover task

        # For hover task, we use a simple kinematic model
        # Since action is already a displacement [dx, dy, dz], the dynamics is just: next_state = state + action
        # We'll create a simple identity model
        self._dynamics_model = HoverKinematicModel()
        self._dynamics_model.eval()

        self._env = PandaHoverEnv(debug=render, render_non_push_motions=False, 
                                 camera_heigh=800, camera_width=800, 
                                 render_every_n_steps=20, visualizer=visualizer)
        self._controller = PushingController(self._env, self._dynamics_model,
                                            cost_func, 
                                            num_samples=1000, horizon=20,
                                            device=device)
    ...
    def run(self):
        ...
        if self._target_state is not None:
            ...
            target_state = get_random_hover_target()
        
        if self._opt_type == "test":
            status = 'random' if self._random_target else 'fixed'
            print(f"Testing hover optimizer with {status} target for {self._epoch} epochs")
            parameters = self.test_params
            for epoch_idx in range(self._epoch):
                if self._random_target:
                    target_state = get_random_hover_target()
                else:
                    target_state = self._target_state.copy()
                ...
                print(f"Epoch {epoch_idx+1}: Target hover position: {target_state}")
                
                end_state, hover_step = run_hover_task(self._env, self._controller, self._n_step, target_state, parameters)
                goal_distance = np.linalg.norm(end_state - target_state)  # 3D distance for hover
                goal_reached = goal_distance < self._goal_tol
                
                print(f"  Final state: {end_state}, Distance: {goal_distance:.4f}, Steps: {hover_step}")
                
                cost = self._compute_cost(goal_distance, goal_reached, hover_step)
                self._logger.update(cost, hover_step, goal_distance, goal_reached, parameters)
            optimized_param = parameters

        else:
            opt_epoch = self._epoch // 3 if self._opt_type=="cma" else self._epoch
            print(f"Optimizing hover task using {self._opt_type.upper()} optimizer for {self._epoch} epoches")
            for _ in range(opt_epoch):
                start_time = time()
                parameters = self._optimizer.suggest()
                suggest_time = time() - start_time
                ...
                for param in parameters:
                    if self._random_target:
                        target_state = get_random_hover_target()
                    end_state, hover_step = run_hover_task(self._env, self._controller, self._n_step, target_state, param)
                    goal_distance = np.linalg.norm(end_state - target_state)  # 3D distance for hover
                    goal_reached = goal_distance < self._goal_tol
                    cost = self._compute_cost(goal_distance, goal_reached, hover_step)

                    self._logger.update(cost, hover_step, goal_distance, goal_reached, param)
                    self._optimizer.register(param, cost)
...
def get_random_hover_target(low=[0.4, -0.3, 0.15], high=[0.7, 0.3, 0.3]):
    """Generate random hover target position [x, y, z]"""
    return np.random.uniform(low=low, high=high, size=None)
...
def run_hover_task(env, controller, step, target_state, hyperparameters):
    """Run hover task - similar to pushing task but for hovering"""
    # init env
    env.set_target_state(target_state)
    state = env.reset()
    # init controller
    ...
    # Debug: print initial state and target
    initial_dist = np.linalg.norm(state - target_state)
    # print(f"Initial state: {state}, Target: {target_state}, Distance: {initial_dist:.4f}")
    
    for i in range(step):
        action = controller.control(state)
        state, _, done, _ = env.step(action)
        if done:
            # Only break if we reached the goal, not if we went out of bounds
            dist = np.linalg.norm(state - target_state)
            if dist < env._goal_tol if hasattr(env, '_goal_tol') else dist < 0.05:
                break  # Reached goal
            # Otherwise continue even if done (might be out of bounds, but let's try to recover)
    end_state = env.get_state()
    return end_state, i
